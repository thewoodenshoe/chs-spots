...
function buildHtml(analytics, pipelineData) {
  ...
  // Umami /stats returns { pageviews, visitors, visits, bounces } as numbers (not { value })
  const statVal = (s, key) => (s && typeof s[key] === 'number' ? s[key] : (s && s[key]?.value) ?? '–');
  const statsHtml = analytics.stats
    ? `
      <div class="stats-grid">
        <div class="stat-card">
          <div class="stat-value">${statVal(analytics.stats, 'pageviews')}</div>
          <div class="stat-label">Pageviews (24h)</div>
        </div>
        <div class="stat-card">
          <div class="stat-value">${statVal(analytics.stats, 'visitors')}</div>
          <div class="stat-label">Visitors (24h)</div>
        </div>
        <div class="stat-card">
          <div class="stat-value">${statVal(analytics.stats, 'visits')}</div>
          <div class="stat-label">Sessions (24h)</div>
        </div>
        <div class="stat-card">
          <div class="stat-value">${statVal(analytics.stats, 'bounces')}</div>
  ...
  const weeklyStatsHtml = analytics.weeklyStats
    ? `
      <div class="stats-grid">
        <div class="stat-card">
          <div class="stat-value">${statVal(analytics.weeklyStats, 'visitors')}</div>
          <div class="stat-label">Visitors (7d)</div>
        </div>
        <div class="stat-card">
          <div class="stat-value">${statVal(analytics.weeklyStats, 'pageviews')}</div>
  ...
  const monthlyStatsHtml = analytics.monthlyStats
    ? `
      <div class="stats-grid">
        <div class="stat-card">
          <div class="stat-value">${statVal(analytics.monthlyStats, 'visitors')}</div>
          <div class="stat-label">Visitors (30d)</div>
        </div>
        <div class="stat-card">
          <div class="stat-value">${statVal(analytics.monthlyStats, 'pageviews')}</div>
...
<title>CHS Finds – Daily Report</title>
<style>
...
<footer>Auto-generated by CHS Finds analytics • ${now.toISOString()}</footer>
</body>
</html>`;
}
...
async function main() {
  ...
  if (token) {
    ...
    try {
      const eventsData = await umamiGet(token, `/events/series?startAt=${oneWeekAgo.getTime()}&endAt=${now.getTime()}&unit=day`);
      if (Array.isArray(eventsData)) {
        const byName = {};
        eventsData.forEach((e) => {
          const name = e.x || e.eventName || 'unknown';
          byName[name] = (byName[name] || 0) + (e.y ?? e.count ?? 0);
        });
        analytics.events = Object.entries(byName).map(([eventName, count]) => ({ eventName, y: count }));
      } else {
        analytics.events = [];
      }
  ...
  if (sendTelegram && TELEGRAM_TOKEN && TELEGRAM_CHAT) {
    try {
      ...
      const lines = [
        ...
        '',
        `Visitors (24h): ${analytics.stats && typeof analytics.stats.visitors === 'number' ? analytics.stats.visitors : (analytics.stats?.visitors?.value ?? '-')}`,
        `Pageviews (24h): ${analytics.stats && typeof analytics.stats.pageviews === 'number' ? analytics.stats.pageviews : (analytics.stats?.pageviews?.value ?? '-')}`,
        `Visitors (7d): ${analytics.weeklyStats && typeof analytics.weeklyStats.visitors === 'number' ? analytics.weeklyStats.visitors : (analytics.weeklyStats?.visitors?.value ?? '-')}`,
