...
/**
 * Openings Step 1+2: RSS feeds + wide LLM web search for new/upcoming venues.
 * Uses loose prompt — step 3 handles quality control.
...
const { log, warn, close: closeLog } = createLogger('op-discover');
...
async function main() {
  ...
  log(`=== Openings Discover: ${getTodayLabel()} ===`);
  ...
  for (const item of llmResults) {
    if (item.name) allCandidates.push({
      name: item.name.trim(), classification: item.classification || 'Coming Soon',
