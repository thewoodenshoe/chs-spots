...
/**
 * Coming Soon Step 7: Pre-report audit on all coming_soon venues.
 * Checks for missing photo, website, description, area. Tries to fix.
...
const { log, warn, close: closeLog } = createLogger('cs-precheck');
...
async function main() {
  ...
  log('=== Coming Soon Pre-Report Check ===');
  db.setAuditContext('pipeline', 'cs-precheck');

  const csVenues = db.getDb().prepare(
    "SELECT * FROM venues WHERE venue_status = 'coming_soon'",
  ).all();
  log(`[precheck] Auditing ${csVenues.length} coming_soon venues`);

  let fixed = 0;
  let photoFixed = 0;
  const flagged = [];

  for (const v of csVenues) {
  ...
  writeStepOutput(PIPELINE, 'step-7-precheck', {
    ...input, precheckAudited: csVenues.length,
