...
/**
 * Recently Opened Step 1+2: RSS feeds + wide LLM web search.
 * Filters for "Recently Opened" classification only.
...
const { log, warn, close: closeLog } = createLogger('ro-discover');
...
function extractFromRss(articles) {
  ...
  for (const article of articles) {
    ...
    if (classification !== 'Recently Opened') continue;
  ...
  log(`[rss] ${candidates.length} recently-opened candidates from RSS`);
  return candidates;
}

async function discoverViaLlm() {
  if (!getApiKey()) { log('[llm] No API key — skipping LLM discovery'); return { items: [], error: false }; }
  const prompt = loadPrompt('recently-opened/step-1-discover', {
    AREAS_PLACEHOLDER: VALID_AREAS.map(a => `"${a}"`).join(', '),
  });
  log('[llm] Calling Grok (wide search for recently opened)...');
...
async function main() {
  ...
  log(`=== Recently Opened Discover: ${getTodayLabel()} ===`);
  ...
  for (const item of llmResults) {
    if (item.name) allCandidates.push({
      name: item.name.trim(), classification: 'Recently Opened',
      ...
      source: (item.source || 'Grok').trim(), openedDate: (item.opened_date || '').trim() || null,
