...
/**
 * Recently Opened Step 7: Pre-report audit on all recently_opened venues.
 * Checks for missing photo, website, description, area, address. Tries to fix.
 * Reads: step-6-aging.json → Outputs: step-7-precheck.json
 */
...
const { log, warn, close: closeLog } = createLogger('ro-precheck');
...
async function tryFixViaLlm(venue) {
...
async function main() {
  const input = readStepOutput(PIPELINE, 'step-6-aging')
    || readStepOutput(PIPELINE, 'step-5-upserted');
  if (!input) { log('No aging/upsert output — aborting'); process.exit(1); }
  log('=== Recently Opened Pre-Report Check ===');
  db.setAuditContext('pipeline', 'ro-precheck');

  const roVenues = db.getDb().prepare(
    "SELECT * FROM venues WHERE venue_status = 'recently_opened'",
  ).all();
  log(`[precheck] Auditing ${roVenues.length} recently_opened venues`);

  let fixed = 0;
  let photoFixed = 0;
  const flagged = [];

  for (const v of roVenues) {
    ...
    if (needsLlm) {
      ...
      const llmData = await tryFixViaLlm(v);
      if (llmData) {
        const updates = {};
  ...
  writeStepOutput(PIPELINE, 'step-7-precheck', {
    ...input, precheckAudited: roVenues.length,
