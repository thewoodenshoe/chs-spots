...
/**
 * Recently Opened Step 3: Geocode, deduplicate, review count filter, and LLM verify.
 * Key difference from coming-soon: uses MAX_REVIEWS = 30 filter since
 * recently opened venues should have very few Google reviews.
...
const { log, warn, close: closeLog } = createLogger('ro-validate');
...
function deduplicateCandidates(geocoded) {
  ...
  const statusVenues = allVenues.filter(v => v.venue_status === 'recently_opened');
  ...
  return geocoded.filter(c => {
    ...
    if (checkReviewCount(c.userRatingsTotal || 0, 'Recently Opened') === 'established') {
...
async function llmVerify(candidates) {
  ...
  const prompt = loadPrompt('recently-opened/step-3-validate', { NAME_LIST: nameList });
  ...
  return candidates.filter(c => {
    ...
    c.openedDate = v.opened_date || c.openedDate || null;
...
async function main() {
  ...
  log(`=== Recently Opened Validate: ${input.candidates.length} candidates ===`);
  db.setAuditContext('pipeline', 'ro-validate');
